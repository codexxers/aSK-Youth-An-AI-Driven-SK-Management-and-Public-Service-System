...
- [ ] `system_logs` table exists in SQLite after server restart
- [ ] Successful login writes a `login_success` log entry with actor, role, IP
- [ ] Failed login attempt writes a `login_fail` log entry
- [ ] Event creation writes a `create_event` log entry with event title as target
- [ ] Suggestion status change writes a `update_suggestion` log entry
- [ ] `GET /api/admin/stats` with Admin token returns `{ total_events, total_attendees, total_budget, pending_suggestions, active_users }` — HTTP 200
- [ ] `GET /api/admin/stats` with Officer token returns HTTP 403
- [ ] `GET /api/admin/logs` with Admin token returns paginated log array
- [ ] `GET /api/admin/logs` with Chairman token returns HTTP 403
- [ ] `GET /api/admin/participation` returns events grouped by SK program (admin/chairman — HTTP 200)
- [ ] `GET /api/admin/budget` returns budget grouped by category (admin/chairman — HTTP 200)
- [ ] `POST /api/users` with Admin token creates a new user with hashed password — HTTP 201
- [ ] `POST /api/users` with Chairman token returns HTTP 403
- [ ] `DELETE /api/users/:id` with Admin token deactivates user — HTTP 200
- [ ] `DELETE /api/users/:id` with Chairman token returns HTTP 403
- [ ] Admin Dashboard renders stat cards with live data (not zeros or placeholders)
- [ ] Participation bar chart renders without JS errors
- [ ] Budget donut chart renders without JS errors
- [ ] System Logs section visible to Admin, hidden to Chairman
- [ ] System Logs table shows actor, role, action, IP for each entry
- [ ] "Export Logs" button downloads a CSV file of currently visible log rows
- [ ] User Management: Admin sees Edit/Delete buttons on user rows; Chairman sees table but no action buttons
- [ ] `CONTEXT.md` updated; `SYSTEM_CONTEXT.md` §3 actors, §4 features, §6 routes updated
